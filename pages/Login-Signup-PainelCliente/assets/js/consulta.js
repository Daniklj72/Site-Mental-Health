...
$(document).ready(function() {
    ...
    // Reagendar consulta - usando event delegation para elementos criados dinamicamente
    $(document).on('click', '.reschedule-btn', function() {
        ...
        // Remover a consulta atual da lista (será reagendada)
        $(this).closest('.appointment-card').remove();
        
        // Se não houver mais consultas, mostrar a mensagem
        if ($('.appointment-card').length === 0) {
            $(".no-appointments").show();
        }
    });

    // Cancelar consulta - usando event delegation para elementos criados dinamicamente
    ...
    // Confirm booking
    $("#confirm-booking").click(function() {// fazer tudo em uma função
